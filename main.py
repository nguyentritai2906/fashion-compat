...
def train(train_loader, tnet, criterion, optimizer, epoch):
    ...
    tnet.train()
    for batch_idx, (img1, desc1, has_text1, img2, desc2, has_text2, img3,
                    desc3, has_text3, condition) in enumerate(train_loader):
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename,
                        'runs/%s/' % (args.name) + 'model_best.pth.tar')
...
class TrainData():
    ...
    def __init__(self, images, text, has_text, conditions=None):
        ...
        images, text, has_text = Variable(images), Variable(text), Variable(
            has_text)
